...
- [x] **Arnés de Auditoría Forense de Privacidad en Modo Incógnito (`PrivacyAuditActivity` - Herramienta Independiente):**
  - Activity independiente de diagnóstico forense desacoplada de la navegación estándar, inspirada en las herramientas de laboratorio de Mozilla y Brave.
  - **Auditoría de Base de Datos Local (Room Leak Check):** Verificación matemática en tiempo real para certificar que ninguna sesión privada escriba registros residuales en `history_entries`, `cookies` o `tabs`.
  - **Auditoría de Memoria y Ciclo de Vida de GeckoView:** Inspección de flags `isPrivate = true`, contextos efímeros en RAM (`contextId`), comprobación de purga de cachés volátiles (`storageController.clearData`) y recolección forzada de basura (`System.gc()`).
  - **Monitor de Fugas de Red y DNS (Network & DoH Inspector):** Comprobación de resolución estricta DoH (DNS sobre HTTPS con Cloudflare/Mozilla TRR), detección de fugas WebRTC (STUN/TURN) y bloqueo de tráfico no cifrado.
  - **Inspector de Huella Digital (Fingerprint Discrepancy Test):** Auditoría activa de consistencia de defensas RFP (Resist Fingerprinting estilo Tor), validando que APIs de canvas, audio, sensores y User-Agent genérico no filtren la identidad biométrica del teléfono.
  - **Verificador de Fugas hacia el Proveedor de Internet (ISP Query Leak Sniffer):** Monitoreo de peticiones para certificar que los términos de búsqueda y dominios no sean interceptados ni expuestos al ISP u operador móvil.
  - **Salida Forense en Crudo y Portapapeles:** Desglose técnico de errores en texto crudo, trazas de pila detalladas (Stack Traces) y botón de copiado rápido al portapapeles con un solo toque para depuración directa en el teléfono.
