...
19. **Arnés de Auditoría Forense de Privacidad en Modo Incógnito (`PrivacyAuditActivity.kt`):**
    - **Herramienta Independiente de Laboratorio:** Diseñada como una `ComponentActivity` desacoplada del ciclo de vida de navegación principal (similar a `CrashInspectorActivity`), permitiendo ejecutar pruebas forenses profundas y estrés de privacidad sin interferir en la sesión del usuario.
    - **Auditoría de Base de Datos Local (Room Leak Check):** Realiza comprobaciones cruzadas contra `BrowserDatabase` durante y después de sesiones privadas para certificar que `history_entries`, `cookies` y `tabs` mantengan 0 inserciones asociadas a dominios de incógnito.
    - **Auditoría de Memoria y Ciclo de Vida de GeckoView:** Inspecciona en tiempo real las instancias de `GeckoSession` para validar la presencia de la bandera `isPrivate = true`, contextos de aislamiento efímeros en RAM (`contextId`), la purga forzada con `storageController.clearData` y la recolección de basura con `System.gc()`.
    - **Monitor de Fugas de Red y DNS (Network & DoH Inspector):** Audita que el túnel de resolución de nombres de dominio emplee TRR con Cloudflare/Mozilla (`TRR_MODE_FIRST`), valida el cierre hermético de WebRTC (`media.peerconnection = false`) y detecta cualquier paquete que intente salir en texto claro o eludir el protocolo HTTPS.
    - **Inspector de Huella Digital (Fingerprint Discrepancy Test):** Realiza comparativas entre los valores de hardware y los reportados al motor web para certificar que el blindaje RFP (Resist Fingerprinting) ofusque la resolución de pantalla, canvas, APIs de audio, fuentes y mantenga un User-Agent genérico estándar.
    - **Monitor de Fugas hacia el Proveedor de Internet (ISP Query Leak Sniffer):** Monitorea la capa de resolución de consultas de búsqueda para verificar si los términos buscados o las URLs navegadas sufren fugas hacia los servidores DNS del proveedor de telefonía móvil (ISP) o si se mantienen totalmente cifrados e indescifrables.
    - **Salida de Diagnóstico en Crudo y Portapapeles:** En caso de fallas o discrepancias, expone la información técnica en crudo (Raw Stack Trace, volcado de memoria y JSON de auditoría) con botón de copiado directo al portapapeles de Android para facilitar la depuración rápida desde el dispositivo móvil.
